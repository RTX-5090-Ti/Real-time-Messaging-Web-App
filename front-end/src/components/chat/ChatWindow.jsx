  ...
  return (
    <main className="flex flex-col flex-1 min-w-0 bg-zinc-50">
        ...
        <div className="flex items-center gap-2">
          ...
          {isSearchOpen && chat ? (
            ...
            >
              ...
              {/* {debouncedQuery && !matchIds.length ? (
                <div className="min-w-[120px] text-xs text-zinc-500 text-right">
                  {isSearching
                    ? "Searching..."
                    : !searchQuery
                    ? ""
      ...
      <div
        ...
        className="relative flex-1 p-6 space-y-5 overflow-y-auto"
      >
            ...
            {messages.map((m, idx) => {
              ...
              return (
                <div key={m.id} data-msg-id={m.id} className="space-y-1">
                  <MessageBubble
                    msg={m}
                    highlightQuery={searchQuery} // mark text (mọi match)
                    isSearchHit={!!searchQuery && matchIds.includes(m.id)} // match hay không
                    isActiveSearchHit={
                      !!searchQuery && matchIds[matchPos] === m.id
                    } // match đang focus
                    onMediaLoad={() => {
                      if (atBottomRef.current) scrollToBottom(false);
                    }}
      ...
      {/* Composer */}
      <Composer chat={chat} onChooseSticker={onChooseSticker} {...composer} />
    </main>
  );
}
